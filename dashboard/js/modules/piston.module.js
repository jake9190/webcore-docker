	...
	$scope.init = function() {
		...
		dataService.getPiston($scope.pistonId, true).then(function (response) {
			...
			try {
				...
				var defaultCategory = ($scope.instance && $scope.instance.settings && ($scope.instance.settings.defaultCategory !== undefined) && ($scope.instance.settings.defaultCategory !== null) && ($scope.instance.settings.defaultCategory !== '')) ? ('' + $scope.instance.settings.defaultCategory) : '0';
				$scope.category = $scope.meta.category ? $scope.meta.category : defaultCategory;
